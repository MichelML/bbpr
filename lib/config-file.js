...
function openConfig () {
  if (globalVars && globalVars.openFileCommand && typeof globalVars.openFileCommand === 'string') {
    shell.exec(`${globalVars.openFileCommand} ${configPath}/bbpr.config.js`)
  } else {
...
function resetConfig () {
  co(function * () {
    ...
    if (shouldReset) {
      shell.cp(`${__dirname.replace(/lib$/, 'config_presets')}/reset.js`, `${configPath}/bbpr.config.js`)
    }
...
function setAllTrueConfig () {
  co(function * () {
    ...
    if (shouldReset) {
      shell.cp(`${__dirname.replace(/lib$/, 'config_presets')}/all-true.js`, `${configPath}/bbpr.config.js`)
    }
...
function setTailoredConfig (pathToFile) {
  co(function * () {
    ...
    if (shouldReset) {
      shell.cp(pathToFile, `${configPath}/bbpr.config.js`)
    }
