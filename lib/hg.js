...
function createRemoteBranch (info) {
  const hgPush = shell.exec(`hg push https://${info.usernameBitBucket}:${info.passwordBitBucket}@bitbucket.org/${info.usernameBitBucketAPI}/${getRepositoryName()} --new-branch -b ${getCurrentBranchName()}`, { silent: true })
  if (hgPush.stderr) outputError(hgPush.stderr)
}
