...
const vcsPath = shell.exec(vcs === 'git' ? 'git remote get-url origin' : 'hg path default', { silent: true })
...
: shell.exec(`hg push https://${info.usernameBitBucket}:${info.passwordBitBucket}@bitbucket.org/${info.usernameBitBucketAPI}/${getRepositoryName()} --new-branch -b ${getCurrentBranchName()}`, { silent: true }))
const repositoryUrl = vcsPath.stdout.trim()

function createRemoteBranch (info) {
  const vcsPushResult = vcsPush()
