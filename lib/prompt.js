...
function * promptDemo () {
  if (config.demo && config.demo.shouldPrompt) {
    ...
    if (needADemo) {
      ...
      let repoNameForDemo = vcs.getRepositoryName().split('-')
      repoNameForDemo = repoNameForDemo.length === 2
        ? repoNameForDemo.join('-')
        : `${repoNameForDemo[0]}-${repoNameForDemo[1][0]}${repoNameForDemo[2][0]}`

      let demoLink = `${config.demo.basePath}/${repoNameForDemo}/${vcs.getCurrentBranchName()}/?dev#${demoHash ? hash.cleanHash(demoHash) : ''}`
