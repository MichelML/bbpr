...
function * promptPassword () {
  if (!config.user || config.user.password === null || !config.user.cachePwd) {
...
function * promptDemo () {
  if (config.demo && config.demo.shouldPrompt) {
    ...
    if (needADemo) {
      ...
      const repoNameForDemo = hg.getRepositoryName().split('-')
...
function * promptIsAllInfoCorrect () {
  presentInfoReview(info.destinationBranch, info.pullRequestTitle, info.pullRequestDescription, info.allReviewers, info.usernameBitBucket)
