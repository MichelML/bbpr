...
function buildPullRequest (info) {
  ...
  const postRequestUrl = `${str.bitBucketAPIUrl.replace('{authentication}', userNamePassword)}/${info.usernameBitBucketAPI}/${vcs.getRepositoryName()}/pullrequests`
...
function getPullRequestJSON (info) {
  return JSON.stringify({
    ...
    'source': {
      ...
      'repository': {
        'full_name': `${info.usernameBitBucketAPI}/${vcs.getRepositoryName()}`
      }
    },
    'destination': {
      'branch': {
