...
function buildPullRequest (pullRequestTitle, pullRequestDescription, destinationBranch, allReviewers, usernameBitBucketAPI, usernameBitBucket, passwordBitBucket) {
  const pullRequestJSON = getPullRequestJSON(pullRequestTitle, pullRequestDescription, destinationBranch, allReviewers, usernameBitBucketAPI)
  const userNamePassword = `${usernameBitBucket}:${passwordBitBucket}`
  const postRequestUrl = `${str.bitBucketAPIUrl}/${usernameBitBucketAPI}/${hg.getRepositoryName()}/pullrequests`
  const postRequest = `${str.curlOptions} '${str.contentTypeJSON}' --user ${userNamePassword} ${postRequestUrl} -d '${pullRequestJSON}'`

  return postRequest
}

function getPullRequestJSON (title, description, destinationBranch, allReviewers, usernameBitBucketAPI) {
  return JSON.stringify({
    ...
    'source': {
      ...
      'repository': {
        'full_name': `${usernameBitBucketAPI}/${hg.getRepositoryName()}`
      }
    },
    'destination': {
      'branch': {
