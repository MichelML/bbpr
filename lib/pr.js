...
exports.buildPullRequest = (pullRequestTitle, pullRequestDescription, destinationBranch, allReviewers, passwordBitBucket) => {
    const pullRequestJSON = getPullRequestJSON(pullRequestTitle, pullRequestDescription, destinationBranch, allReviewers);
    const userNamePassword = `${config.user.name}:${passwordBitBucket}`;
...
function getPullRequestJSON(title, description, destinationBranch, allReviewers) {
    return JSON.stringify({
        ...
        "close_source_branch": config.sourceBranch.close
    });
}
