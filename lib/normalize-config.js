...
function normalizeConfig (config) {
  if (!config.organization) config.organization = { name: hg.getOrganizationName() }
  if (!config.organization.name) config.organization.name = hg.getOrganizationName()
  if (!config.user) config.user = { name: hg.getUserName(), password: null, cachePwd: false }
  if (!config.user.name) config.user.name = hg.getUserName()
